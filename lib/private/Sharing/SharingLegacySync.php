...
final class SharingLegacySync implements IEventListener {
	...
	public function mapLegacyShares(?IUser $user, ?int $limit = null): void {
		...
		foreach ($rows as $legacyId) {
			...
			$legacyMapping = $this->createLegacyMapping($id, $provider, $legacyId, new DateTimeImmutable());
	...
	public function handle(Event $event): void {
		...
		if (
			...
			$legacyMapping = $this->getLegacyMappingByLegacyProviderAndId($legacyProvider, $legacyId);
			...
			if ($event instanceof ShareDeletedEvent) {
				...
				if ($legacyMapping instanceof LegacyMapping) {
					$this->wrapSync($legacyMapping->id, function () use ($legacyMapping): void {
						...
						$this->deleteLegacyMappings($legacyMapping->id);
					});
				}
			} else {
				if ($legacyMapping instanceof LegacyMapping) {
					$this->wrapSync($legacyMapping->id, function () use ($legacyMapping): void {
						...
						$legacyMappings = $this->getLegacyMappings($legacyMapping->id);
					...
					$this->wrapSync($id, function () use ($legacyProvider, $legacyId, $id): void {
						$legacyMapping = $this->createLegacyMapping($id, $legacyProvider, $legacyId, new DateTimeImmutable());
						$this->syncLegacySharesToShare($id, [$legacyMapping]);
					});
				}
			}
		...
		if ($event instanceof SharesDeletedEvent) {
			foreach ($event->shareIds as $id) {
				$this->wrapSync($id, function () use ($id): void {
					$legacyMappings = $this->getLegacyMappings($id);
	...
	private function createLegacyMapping(string $id, string $legacyProvider, int $legacyId, DateTimeImmutable $lastUpdated): LegacyMapping {
		$qb = $this->dbConnection->getQueryBuilder();
		$qb
	...
	private function getLegacyMappingByLegacyProviderAndId(string $legacyProvider, int $legacyId): ?LegacyMapping {
		$qb = $this->dbConnection->getQueryBuilder();
		$result = $qb
		...
		/** @var array{id: int, last_updated: int}|false $row */
		$row = $result->fetch();
		if ($row === false) {
			return null;
		}
	 ...
	 */
	private function getLegacyMappings(string $id): array {
		$legacyMappings = [];

		$qb = $this->dbConnection->getQueryBuilder();
		$result = $qb
			->select('id', 'legacy_provider', 'legacy_id', 'last_updated')
			->from('sharing_share_legacy_mapping')
			->where($qb->expr()->eq('id', $qb->createNamedParameter($id)))
			->executeQuery();

		/** @var list<array{id: int, legacy_provider: string, legacy_id: int, last_updated: int}> $rows */
	...
	private function deleteLegacyMappings(string $id): void {
		$qb = $this->dbConnection->getQueryBuilder();
		$qb
	...
	private function updateLegacyMapping(LegacyMapping $legacyMapping): void {
		$qb = $this->dbConnection->getQueryBuilder();
		$qb
	...
	private function syncLegacySharesToShare(string $id, array $legacyMappings): void {
		...
		foreach ($legacyMappings as $legacyMapping) {
			...
			$recipients[$recipientTypeClass][$uniqueId] ??= new ShareRecipient(
				...
				$recipient['value'],
				$recipient['remote'],
				$isTokenRecipient ? $legacyShare->getToken() : $this->getSharingManager()->generateSecret(), // TODO: Secret must be stored and not generated.
				new ShareUser(
		...
		$properties = [	];
		if (($expirationDate = $legacyShares[0]->getExpirationDate()) !== null) {
			$properties[] = new ShareProperty(ExpirationDateSharePropertyType::class, $expirationDate->format(DateTimeInterface::ATOM));
		}

		if (($password = $legacyShares[0]->getPassword()) !== null) {
			$properties[] = new ShareProperty(PasswordSharePropertyType::class, $password);
		}

		if (($label = $legacyShares[0]->getLabel()) !== '') {
			$properties[] = new ShareProperty(LabelSharePropertyType::class, $label);
		}

		if (($note = $legacyShares[0]->getNote()) !== '') {
	...
	private function syncShareToLegacyShares(Share $share): void {
		...
		$legacyShares = [];
		foreach ($this->getLegacyMappings($share->id) as $legacyMapping) {
			$legacyShare = $legacyMapping->getLegacyShare($this->legacySharingManager);
		...
		foreach ($share->recipients as $recipient) {
			...
			foreach ($share->sources as $source) {
				...
				if (($legacyMapping = $legacyMappings[$recipient->class][$recipient->value][$source->value] ?? null) !== null) {
					...
					$this->updateLegacyMapping(new LegacyMapping(
						$share->id,
	...
	private function legacyPermissionsToPermissions(int $legacyPermissions, bool $allowDownload): array {
		...
		foreach ([
			...
			if (($legacyPermissions & $mask) === $mask) {
				$permissions[] = new SharePermission($permissionTypeClass, true);
			}
		}

		if ($allowDownload) {
			$permissions[] = new SharePermission(NodeDownloadSharePermissionType::class, true);
		}
